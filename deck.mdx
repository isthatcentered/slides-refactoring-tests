...
- Après: 0 et 5 = array length 
- Sais retourne tableau, il devrait contenir 10 items il est vide
- Bon matcher donne indices utiles
...
Memory dump c'est quoi ? C'est ça: (next slide)
...
- Dump tableaux aka débrouille toi
- A nous comparer tableau a vs tableau B
- Styles aident pas mais erreur bien affichée. Juste chiant à trouver
- Agassant en cas de fail imprévue
...
- Simplifier résultat
- Order by id
- Donc prends id et matche contre table number
...
- Vérifie objet vs objet
- Comment mieux .
- Cherche vérifier bon objet récupéré
- Besoin pouvoir identifier si récup A ou B
- Transfo résult
...
- Checks "validate()" retourne toutes erreurs trouvée dans propriété erreur de report 

<br/>

- Only care about shape of error
- Veut seulement vérifier que clé erreur comprenant résultat chaque validateur
...
- Avant: 
- Test connait plus que ce dont il a besoin
- Casse changement message validateur email
- Casse ajout/rename clé report

<br/>

- Après:
- Test seullement ce qui compte pour ce test
- Peut évoluer autour, test continue passer
- (Attention matcher permissif. Vérifie ce qui est spécifié présent, autorise tout le reste)
