...
- Donc pkoi refactorer ? Parce que si test fait peur -> test saute
...
- ++ checks manuels -- checks auto
- Dur penser à tout donc
...
- Mise en danger autres test
- ++ y a code à un endroit, ++ danger casser en changeant qqc (ici, désactiver)
- Surface à apprendre très large, doit comprendre tt test pour cibler ce qui m'intéresse 
...
- Grandes chances finir avec bouts désactivés
- Pour modifier, doit connaitre tout test et tout code
...
- Veut comprendre failure via chaine assertions -> actions -> setup
- Ici mélangées
...
- Bien pouvoir retrouver rapidement étapes test
- Mais manière tester change à chaque fois, pas suffisant


- Licorne = test pas comme les autres (sans bonnes raison)
...
- structure
- Conventions
- Capables retrouver marques ds test rapidement
...
<Chapter emojis="📚🤔" title="Imperative recipe" />
